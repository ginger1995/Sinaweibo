...
class SQLiteDB {
	//let DB_NAME = "ScanGoodsBarCode.db"
    ...
    class func sharedInstance(dbName:String) -> SQLiteDB! {
		dispatch_once(&Static.token) {
			Static.instance = self.init(dbName:dbName,gid:"")
		}
		return Static.instance!
	}
	
	class func sharedInstance(dbName:String,gid:String) -> SQLiteDB! {
		dispatch_once(&Static.token) {
			Static.instance = self.init(dbName:dbName,gid:gid)
		}
		return Static.instance!
	}
 
	required init(dbName:String,gid:String) {
		...
		let dbName:String = String.fromCString(dbName)!
		var docDir = ""
		// Is this for an app group?
		if GROUP.isEmpty {
			// Get path to DB in Documents directory
			docDir = NSSearchPathForDirectoriesInDomains(.CachesDirectory, .UserDomainMask, true)[0]
		} else {
		...
		if !(fm.fileExistsAtPath(path)) {
			...
			if let from = (NSBundle.mainBundle().pathForResource(dbName, ofType: nil)) {
				var error:NSError?
				do {
    ...
    func query(sql:String, parameters:[AnyObject]?=nil,backCall:(rows:[SQLRow]) -> Void) {
        var rows = [SQLRow]()
